...
public final class XdmSaxon implements SXdm {
    ...
    private final UncheckedIOScalar<XdmValue> val;
    ...
    public XdmSaxon(Input input) {
        this(() -> DOC_BUILDER.build(new StreamSource(input.stream())));
    }
    ...
    public XdmSaxon(XQueryCompiler compiler, Scalar<XdmValue> val) {
        this(compiler, new UncheckedIOScalar<>(new IoCheckedScalar<>(new StickyScalar<>(val))));
    }

    public XdmSaxon(XQueryCompiler compiler, UncheckedIOScalar<XdmValue> val) {
